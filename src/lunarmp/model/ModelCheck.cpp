...
namespace lunarmp {
void ModelCheck::checkConnectedComponents(triangular_Mesh mesh) {
    ...
    triangular_Mesh::Property_map<face_descriptor, std::size_t> fccmap = mesh.add_property_map<face_descriptor, std::size_t>("f:CC").first;
    std::size_t num = PMP::connected_components(mesh, fccmap, PMP::parameters::edge_is_constrained_map(Constraint<triangular_Mesh>(mesh, bound)));
    number_of_connected_components = (int)num;
}

void ModelCheck::checkHoles(triangular_Mesh mesh) {
    number_of_holes = 0;
...
void ModelCheck::checkIntersect(triangular_Mesh mesh) {
