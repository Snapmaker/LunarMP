...
struct Duration {
     ...
     */
    Duration operator+(const Duration& other) const { return Duration(value + other.value); };
    Duration operator-(const Duration& other) const { return Duration(value + other.value); };
...
constexpr Duration operator"" _s(const long double seconds) { return Duration(seconds); }
