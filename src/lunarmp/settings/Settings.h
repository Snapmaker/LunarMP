...
// Maximum number of infill layers that can be combined into a single infill
// extrusion area.
...
class Settings
{
  public:
    ...
    void add(const Settings& other_settings);
    ...
    template <typename A>
    A getOrDefault(const std::string& key, A const value) const
    {
        if (has(key))
        {
            return get<A>(key);
     ...
     * when slicing using LunarTPP from the command line. In theory you could
     * put the output of this command in a call to LunarTPP.
