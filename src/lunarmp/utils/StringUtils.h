...
static inline void writeInt2mm(const int32_t coord, std::ostream& ss) {
    ...
    int char_count =
        sprintf(buffer, "%d", coord);  // we haven't found any way for the windows compiler to accept formatting of a coord_t, so it has to be int32_t instead
#ifdef DEBUG
    ...
    if (trailing_zeros == 3) {  // no need to write the decimal dot
    ...
    if (char_count <= 3) {
        ...
        ss << "0.";
        for (int nulls = char_count - start; nulls < 3; nulls++) {  // fill up to 3 decimals with zeros
            ss << '0';
        }
        buffer[char_count - trailing_zeros] = '\0';
        ...
        char prev = '.';
        int pos;
        for (pos = char_count - 3; pos <= end_pos; pos++) {  // shift all characters and insert the decimal dot
...
constexpr static unsigned int str2int(const char* str, int h = 0) { return !str[h] ? 5381 : (str2int(str, h + 1) * 33) ^ str[h]; }
