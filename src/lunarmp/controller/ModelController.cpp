...
void ModelController::simplification(DataGroup& data_group) {
    ...
    auto machine_box = data_group.settings.get<double>("machine_box");
    auto threshold = data_group.settings.get<double>("threshold");

    ModelSimplification model_simplification;
    model_simplification.modelSimplification(input_path, output_path, simplify_type, machine_box, threshold);
}
